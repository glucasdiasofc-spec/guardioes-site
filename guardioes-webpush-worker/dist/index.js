...
var LIMITE_IMAGEM = 10 * 1024 * 1024;
var LIMITE_VIDEO = 20 * 1024 * 1024;
var index_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/upload") {
        const usuario = await autorizarUpload(
          request,
          env
        );
      ...
      return responderJSON(
        {
          ok: false,
          erro: erro.status === 500 || !erro.status ? "Erro interno no servidor de publica\xE7\xF5es." : erro.message
        },
        erro.status || 500
      );
    }
...
__name(cabecalhosCORS, "cabecalhosCORS");
...
__name(responderJSON, "responderJSON");
function criarErro(status, mensagem) {
  const erro = new Error(mensagem);
  erro.status = status;
  return erro;
}
__name(criarErro, "criarErro");
async function autorizarUpload(request, env) {
  const chaveAdmin = request.headers.get(
    "X-Admin-Key"
  );
  if (env.ADMIN_UPLOAD_KEY && chaveAdmin === env.ADMIN_UPLOAD_KEY) {
    return {
      uid: "admin",
      modo: "admin"
    };
  ...
  if (!env.FIREBASE_WEB_API_KEY) {
    throw criarErro(
      500,
      "FIREBASE_WEB_API_KEY n\xE3o foi configurada."
    );
  }
  const idToken = authorization.slice(7).trim();
  const respostaFirebase = await fetch(
  ...
  const dadosFirebase = await respostaFirebase.json();
  const usuario = dadosFirebase.users && dadosFirebase.users[0];
  if (!respostaFirebase.ok || !usuario || !usuario.localId || usuario.disabled) {
    throw criarErro(
      401,
      "Sess\xE3o Firebase inv\xE1lida ou expirada."
    );
  }
  return {
    uid: usuario.localId,
...
__name(autorizarUpload, "autorizarUpload");
async function enviarMidiaTelegram(request, env, usuario) {
  ...
  if (!tokenTelegram || !chatIdTelegram) {
    throw criarErro(
      500,
      "Segredos do Telegram n\xE3o configurados."
    );
  }
  if (!/^\d+:[A-Za-z0-9_-]+$/.test(tokenTelegram)) {
    throw criarErro(
      500,
      "TELEGRAM_BOT_TOKEN inv\xE1lido. No segredo, informe somente o token bruto do BotFather, sem 'bot', aspas ou URL."
    );
  }
  ...
  if (!(arquivo instanceof File)) {
    throw criarErro(
      400,
      "Nenhuma m\xEDdia foi enviada."
    );
  }
  const ehImagem = arquivo.type.startsWith("image/");
  const ehVideo = arquivo.type === "video/mp4" || /\.mp4$/i.test(arquivo.name || "");
  if (!ehImagem && !ehVideo) {
    throw criarErro(
      415,
      "Envie uma imagem ou um v\xEDdeo MP4."
    );
  }
  if (ehImagem && arquivo.size > 10 * 1024 * 1024) {
    throw criarErro(
      413,
      "A imagem deve ter no m\xE1ximo 10 MB."
    );
  }
  if (ehVideo && arquivo.size > 20 * 1024 * 1024) {
    throw criarErro(
      413,
      "O v\xEDdeo deve ter no m\xE1ximo 20 MB."
    );
  }
  const metodo = ehImagem ? "sendPhoto" : "sendVideo";
  const campoArquivo = ehImagem ? "photo" : "video";
  ...
  const urlTelegram = `https://api.telegram.org/bot${tokenTelegram}/${metodo}`;
  ...
  const textoTelegram = await respostaTelegram.text();
  let resultado;
  try {
    resultado = textoTelegram ? JSON.parse(textoTelegram) : {};
  } catch (erroJSON) {
    resultado = {
      ok: false,
      description: "Resposta inv\xE1lida da API do Telegram."
    };
  }
  if (!respostaTelegram.ok || !resultado.ok) {
    console.error(
      "Telegram recusou a publica\xE7\xE3o:",
      {
    ...
    if (respostaTelegram.status === 404 || resultado.description === "Not Found") {
      throw criarErro(
        502,
        "Telegram retornou Not Found. Verifique se TELEGRAM_BOT_TOKEN cont\xE9m somente o token bruto do BotFather e n\xE3o come\xE7a com 'bot'."
      );
    }
    throw criarErro(
      502,
      resultado.description || "O Telegram recusou a m\xEDdia."
    );
  }
  const mensagem = resultado.result;
  const midia = ehImagem ? mensagem.photo[mensagem.photo.length - 1] : mensagem.video;
  return responderJSON({
    ok: true,
    uid: usuario.uid,
    ...
    midia: {
      ...
      telegramFileUniqueId: midia.file_unique_id || null,
...
__name(enviarMidiaTelegram, "enviarMidiaTelegram");
async function entregarMidiaTelegram(request, env, ctx) {
  if (!env.TELEGRAM_BOT_TOKEN) {
    throw criarErro(
      500,
      "Token do Telegram n\xE3o configurado."
    );
  }
  const url = new URL(request.url);
  const fileId = url.searchParams.get(
    "file_id"
  );
  ...
  const respostaGetFile = await fetch(
    "https://api.telegram.org/bot" + env.TELEGRAM_BOT_TOKEN + "/getFile?file_id=" + encodeURIComponent(
      fileId
    )
  );
  const dadosGetFile = await respostaGetFile.json();
  if (!dadosGetFile.ok || !dadosGetFile.result || !dadosGetFile.result.file_path) {
    throw criarErro(
      404,
      dadosGetFile.description || "M\xEDdia n\xE3o encontrada."
    );
  }
  const respostaArquivo = await fetch(
    "https://api.telegram.org/file/bot" + env.TELEGRAM_BOT_TOKEN + "/" + dadosGetFile.result.file_path
  );
  if (!respostaArquivo.ok || !respostaArquivo.body) {
    throw criarErro(
      502,
      "Falha ao carregar a m\xEDdia do Telegram."
    );
  }
  const respostaFinal = new Response(
    respostaArquivo.body,
    {
      status: 200,
      headers: {
        "Content-Type": respostaArquivo.headers.get(
          "Content-Type"
...
__name(entregarMidiaTelegram, "entregarMidiaTelegram");
async function lerAssinaturasPush(env, username) {
  if (!env.PUSH_KV) {
    throw criarErro(500, "Binding PUSH_KV n\xE3o configurado.");
  }
  const chave = `push:user:${normalizarUsername(username)}`;
  return await env.PUSH_KV.get(chave, "json") || [];
}
__name(lerAssinaturasPush, "lerAssinaturasPush");
async function registrarAssinaturaPush(request, env, usuario) {
  ...
  if (!username || !subscription || !subscription.endpoint) {
    throw criarErro(400, "Assinatura Web Push inv\xE1lida.");
  }
...
__name(registrarAssinaturaPush, "registrarAssinaturaPush");
async function enviarPushProprio(request, env, usuario) {
  if (!env.PUSH_KV) {
    throw criarErro(500, "Binding PUSH_KV n\xE3o configurado.");
  }
  if (!env.VAPID_PUBLIC_KEY || !env.VAPID_PRIVATE_KEY || !env.VAPID_SUBJECT) {
    throw criarErro(500, "Segredos VAPID n\xE3o configurados.");
  }
  const dados = await request.json();
  ...
  if (!destinatario || !texto) {
    throw criarErro(400, "Destinat\xE1rio e texto s\xE3o obrigat\xF3rios.");
  }
  ...
  if (!assinaturas.length) {
    return responderJSON({
      ok: true,
      enviados: 0,
      motivo: "Destinat\xE1rio sem dispositivos inscritos."
    });
  }
  ...
  const titulo = remetente ? `Nova mensagem de ${remetente}` : "Nova mensagem";
  const corpo = texto.length > 160 ? `${texto.slice(0, 157)}...` : texto;
  const expiradas = [];
  let enviados = 0;
