...
async function salvarNovaUnidadeAdmin() {
    ...
    try {
        ...
        alert("Sucesso ao criar unidade!");
        nomeInput.value = "";
        if (typeof carregarUnidadesCadastradas === "function") carregarUnidadesCadastradas();
    } catch (e) { 
        alert("Erro ao criar unidade: " + e.message); 
    } finally {
...
// =================================================================
// LÓGICA DE CATEGORIAS E CATÁLOGOS (ESPECIALIDADES, MESTRADOS, CLASSES)
// =================================================================

// Controle de Estado de Categorias (null = tela inicial de categorias, 'TODAS' = agrupado por categorias, string = categoria específica)
let categoriaAtivaEspecialidades = null;
let categoriaAtivaMestrados = null;
let categoriaAtivaClasses = null;

// Caches de Dados
let cacheCatalogoEspecialidades = [];
let cacheCatalogoMestrados = [];
let cacheCatalogoClasses = [];
...
async function abrirCatalogoEspecialidades() {
    ...
    categoriaAtivaEspecialidades = null;

    if (cacheCatalogoEspecialidades.length === 0) {
        try {
            const snap = await window.ClubeDB.textoDB.collection("especialidades").get();
            cacheCatalogoEspecialidades = [];
            snap.forEach(doc => cacheCatalogoEspecialidades.push({ id: doc.id, ...doc.data() }));
        } catch (e) {
...
function selecionarCategoriaEspecialidades(cat) {
    categoriaAtivaEspecialidades = cat;
    renderizarCatalogoEspecialidades();
}

function voltarParaCategoriasEspecialidades() {
    categoriaAtivaEspecialidades = null;
    document.getElementById("busca-especialidade").value = "";
    renderizarCatalogoEspecialidades();
}
...
function renderizarCatalogoEspecialidades() {
    const gridCategorias = document.getElementById("grid-categorias-especialidades");
    const containerItens = document.getElementById("lista-especialidades-container");
    const cabecalhoCat = document.getElementById("cabecalho-categoria-especialidades");
    const nomeCatTxt = document.getElementById("nome-categoria-ativa-especialidade");
    const termoBusca = (document.getElementById("busca-especialidade").value || "").trim().toLowerCase();

    const mapaCategorias = {};
    cacheCatalogoEspecialidades.forEach(item => {
        const cat = item.categoria || item.area || "Geral";
        if (!mapaCategorias[cat]) mapaCategorias[cat] = [];
        mapaCategorias[cat].push(item);
    });
    const listaCategorias = Object.keys(mapaCategorias).sort();

    // SE NÃO HÁ CATEGORIA SELECIONADA E NÃO HÁ PESQUISA: MOSTRA A GRADE DE CATEGORIAS
    if (categoriaAtivaEspecialidades === null && termoBusca === "") {
        if (gridCategorias) gridCategorias.style.display = "grid";
        if (cabecalhoCat) cabecalhoCat.style.display = "none";
        if (containerItens) containerItens.innerHTML = "";

        let htmlGrid = `
            <div onclick="selecionarCategoriaEspecialidades('TODAS')" style="background: linear-gradient(135deg, #0052d4, #4364f7); padding: 14px 10px; border-radius: 10px; text-align: center; cursor: pointer; border: 1px solid #0095f6; transition: transform 0.2s;">
                <div style="font-size: 22px; margin-bottom: 4px;">🌟</div>
                <div style="color: #fff; font-weight: bold; font-size: 13px;">Todas</div>
                <div style="color: rgba(255,255,255,0.8); font-size: 11px; margin-top: 2px;">${cacheCatalogoEspecialidades.length} itens</div>
            </div>
        `;

        listaCategorias.forEach(cat => {
            const qtd = mapaCategorias[cat].length;
            htmlGrid += `
                <div onclick="selecionarCategoriaEspecialidades('${cat.replace(/'/g, "\\'")}')" style="background: #1e1e1e; padding: 14px 10px; border-radius: 10px; text-align: center; cursor: pointer; border: 1px solid #333; transition: transform 0.2s;">
                    <div style="font-size: 22px; margin-bottom: 4px;">📁</div>
                    <div style="color: #fff; font-weight: 600; font-size: 12px; word-break: break-word;">${cat}</div>
                    <div style="color: #8e8e8e; font-size: 11px; margin-top: 2px;">${qtd} item(ns)</div>
                </div>
            `;
        });

        if (gridCategorias) gridCategorias.innerHTML = htmlGrid;
        return;
    }

    // EXIBIÇÃO DOS ITENS (Sua busca ou categoria selecionada)
    if (gridCategorias) gridCategorias.style.display = "none";
    if (cabecalhoCat) cabecalhoCat.style.display = "flex";

    let itensParaExibir = [];

    if (termoBusca !== "") {
        if (categoriaAtivaEspecialidades === null) {
            if (nomeCatTxt) nomeCatTxt.textContent = "Pesquisa: Todas as Categorias";
            itensParaExibir = cacheCatalogoEspecialidades.filter(i => 
                (i.nome || "").toLowerCase().includes(termoBusca) || 
                (i.categoria || i.area || "").toLowerCase().includes(termoBusca)
            );
        } else if (categoriaAtivaEspecialidades === "TODAS") {
            if (nomeCatTxt) nomeCatTxt.textContent = "Categoria: Todas";
            itensParaExibir = cacheCatalogoEspecialidades.filter(i => 
                (i.nome || "").toLowerCase().includes(termoBusca)
            );
        } else {
            if (nomeCatTxt) nomeCatTxt.textContent = `Categoria: ${categoriaAtivaEspecialidades}`;
            itensParaExibir = cacheCatalogoEspecialidades.filter(i => 
                (i.categoria || i.area || "Geral") === categoriaAtivaEspecialidades &&
                (i.nome || "").toLowerCase().includes(termoBusca)
            );
        }
    } else {
        if (categoriaAtivaEspecialidades === "TODAS") {
            if (nomeCatTxt) nomeCatTxt.textContent = "Categoria: Todas";
            itensParaExibir = cacheCatalogoEspecialidades;
        } else {
            if (nomeCatTxt) nomeCatTxt.textContent = `Categoria: ${categoriaAtivaEspecialidades}`;
            itensParaExibir = cacheCatalogoEspecialidades.filter(i => (i.categoria || i.area || "Geral") === categoriaAtivaEspecialidades);
        }
    }

    if (itensParaExibir.length === 0) {
        if (containerItens) containerItens.innerHTML = "<p style='color: #8e8e8e; text-align: center; padding: 20px 0;'>Nenhum item encontrado.</p>";
        return;
    }

    if (categoriaAtivaEspecialidades === "TODAS" || (categoriaAtivaEspecialidades === null && termoBusca !== "")) {
        const agrp = {};
        itensParaExibir.forEach(item => {
            const c = item.categoria || item.area || "Geral";
            if (!agrp[c]) agrp[c] = [];
            agrp[c].push(item);
        });

        let html = "";
        Object.keys(agrp).sort().forEach(cat => {
            html += `<div style="margin-bottom: 20px;">
                <h4 style="color: #0095f6; margin: 0 0 10px 0; font-size: 14px; border-bottom: 1px solid #262626; padding-bottom: 5px;">📁 ${cat}</h4>
                <div style="display: flex; flex-direction: column; gap: 10px;">
                    ${agrp[cat].map(item => renderizarCardItemCatalogo(item, "especialidades")).join("")}
                </div>
            </div>`;
        });
        if (containerItens) containerItens.innerHTML = html;
    } else {
        if (containerItens) {
            containerItens.innerHTML = `
                <div style="display: flex; flex-direction: column; gap: 10px;">
                    ${itensParaExibir.map(item => renderizarCardItemCatalogo(item, "especialidades")).join("")}
                </div>
            `;
        }
    }
...
async function abrirCatalogoMestrados() {
    ...
    categoriaAtivaMestrados = null;

    if (cacheCatalogoMestrados.length === 0) {
        try {
            const snap = await window.ClubeDB.textoDB.collection("mestrados").get();
            cacheCatalogoMestrados = [];
            snap.forEach(doc => cacheCatalogoMestrados.push({ id: doc.id, ...doc.data() }));
        } catch (e) {
            console.error("Erro ao carregar mestrados:", e);
        }
    }
...
function selecionarCategoriaMestrados(cat) {
    categoriaAtivaMestrados = cat;
    renderizarCatalogoMestrados();
}

function voltarParaCategoriasMestrados() {
    categoriaAtivaMestrados = null;
    document.getElementById("busca-mestrado").value = "";
    renderizarCatalogoMestrados();
}
...
function renderizarCatalogoMestrados() {
    const gridCategorias = document.getElementById("grid-categorias-mestrados");
    const containerItens = document.getElementById("lista-mestrados-container");
    const cabecalhoCat = document.getElementById("cabecalho-categoria-mestrados");
    const nomeCatTxt = document.getElementById("nome-categoria-ativa-mestrado");
    const termoBusca = (document.getElementById("busca-mestrado").value || "").trim().toLowerCase();

    const mapaCategorias = {};
    cacheCatalogoMestrados.forEach(item => {
        const cat = item.categoria || item.area || "Geral";
        if (!mapaCategorias[cat]) mapaCategorias[cat] = [];
        mapaCategorias[cat].push(item);
    });
    ...
    if (categoriaAtivaMestrados === null && termoBusca === "") {
        if (gridCategorias) gridCategorias.style.display = "grid";
        if (cabecalhoCat) cabecalhoCat.style.display = "none";
        if (containerItens) containerItens.innerHTML = "";

        let htmlGrid = `
            <div onclick="selecionarCategoriaMestrados('TODAS')" style="background: linear-gradient(135deg, #11998e, #38ef7d); padding: 14px 10px; border-radius: 10px; text-align: center; cursor: pointer; border: 1px solid #28a745; transition: transform 0.2s;">
                <div style="font-size: 22px; margin-bottom: 4px;">🏆</div>
                <div style="color: #fff; font-weight: bold; font-size: 13px;">Todas</div>
                <div style="color: rgba(255,255,255,0.8); font-size: 11px; margin-top: 2px;">${cacheCatalogoMestrados.length} itens</div>
            </div>
        `;
        ...
        listaCategorias.forEach(cat => {
            const qtd = mapaCategorias[cat].length;
            htmlGrid += `
                <div onclick="selecionarCategoriaMestrados('${cat.replace(/'/g, "\\'")}')" style="background: #1e1e1e; padding: 14px 10px; border-radius: 10px; text-align: center; cursor: pointer; border: 1px solid #333; transition: transform 0.2s;">
                    <div style="font-size: 22px; margin-bottom: 4px;">📁</div>
                    <div style="color: #fff; font-weight: 600; font-size: 12px; word-break: break-word;">${cat}</div>
                    <div style="color: #8e8e8e; font-size: 11px; margin-top: 2px;">${qtd} item(ns)</div>
                </div>
    ...
    if (termoBusca !== "") {
        if (categoriaAtivaMestrados === null) {
            if (nomeCatTxt) nomeCatTxt.textContent = "Pesquisa: Todas as Categorias";
            itensParaExibir = cacheCatalogoMestrados.filter(i => 
                (i.nome || "").toLowerCase().includes(termoBusca) || 
                (i.categoria || i.area || "").toLowerCase().includes(termoBusca)
            );
        } else if (categoriaAtivaMestrados === "TODAS") {
            if (nomeCatTxt) nomeCatTxt.textContent = "Categoria: Todas";
            itensParaExibir = cacheCatalogoMestrados.filter(i => 
                (i.nome || "").toLowerCase().includes(termoBusca)
            );
        } else {
            if (nomeCatTxt) nomeCatTxt.textContent = `Categoria: ${categoriaAtivaMestrados}`;
            itensParaExibir = cacheCatalogoMestrados.filter(i => 
                (i.categoria || i.area || "Geral") === categoriaAtivaMestrados &&
                (i.nome || "").toLowerCase().includes(termoBusca)
            );
        }
    } else {
        if (categoriaAtivaMestrados === "TODAS") {
            if (nomeCatTxt) nomeCatTxt.textContent = "Categoria: Todas";
            itensParaExibir = cacheCatalogoMestrados;
        } else {
            if (nomeCatTxt) nomeCatTxt.textContent = `Categoria: ${categoriaAtivaMestrados}`;
            itensParaExibir = cacheCatalogoMestrados.filter(i => (i.categoria || i.area || "Geral") === categoriaAtivaMestrados);
        }
    }
    ...
    if (categoriaAtivaMestrados === "TODAS" || (categoriaAtivaMestrados === null && termoBusca !== "")) {
        const agrp = {};
        itensParaExibir.forEach(item => {
            const c = item.categoria || item.area || "Geral";
            if (!agrp[c]) agrp[c] = [];
        ...
        Object.keys(agrp).sort().forEach(cat => {
            html += `<div style="margin-bottom: 20px;">
                <h4 style="color: #28a745; margin: 0 0 10px 0; font-size: 14px; border-bottom: 1px solid #262626; padding-bottom: 5px;">📁 ${cat}</h4>
                <div style="display: flex; flex-direction: column; gap: 10px;">
                    ${agrp[cat].map(item => renderizarCardItemCatalogo(item, "mestrados")).join("")}
                </div>
            </div>`;
        });
        if (containerItens) containerItens.innerHTML = html;
        ...
        if (containerItens) {
            containerItens.innerHTML = `
                <div style="display: flex; flex-direction: column; gap: 10px;">
                    ${itensParaExibir.map(item => renderizarCardItemCatalogo(item, "mestrados")).join("")}
                </div>
            `;
        }
    }
...
async function abrirCatalogoClasses() {
    ...
    categoriaAtivaClasses = null;

    if (cacheCatalogoClasses.length === 0) {
        try {
            const snap = await window.ClubeDB.textoDB.collection("classes").get();
            cacheCatalogoClasses = [];
            snap.forEach(doc => cacheCatalogoClasses.push({ id: doc.id, ...doc.data() }));
        } catch (e) {
...
function selecionarCategoriaClasses(cat) {
    categoriaAtivaClasses = cat;
    renderizarCatalogoClasses();
}

function voltarParaCategoriasClasses() {
    categoriaAtivaClasses = null;
    document.getElementById("busca-classe").value = "";
    renderizarCatalogoClasses();
}
...
function renderizarCatalogoClasses() {
    const gridCategorias = document.getElementById("grid-categorias-classes");
    const containerItens = document.getElementById("lista-classes-container");
    const cabecalhoCat = document.getElementById("cabecalho-categoria-classes");
    const nomeCatTxt = document.getElementById("nome-categoria-ativa-classe");
    const termoBusca = (document.getElementById("busca-classe").value || "").trim().toLowerCase();

    const mapaCategorias = {};
    cacheCatalogoClasses.forEach(item => {
        const cat = item.categoria || item.area || "Geral";
        if (!mapaCategorias[cat]) mapaCategorias[cat] = [];
        mapaCategorias[cat].push(item);
    });
    ...
    if (categoriaAtivaClasses === null && termoBusca === "") {
        if (gridCategorias) gridCategorias.style.display = "grid";
        if (cabecalhoCat) cabecalhoCat.style.display = "none";
        if (containerItens) containerItens.innerHTML = "";

        let htmlGrid = `
            <div onclick="selecionarCategoriaClasses('TODAS')" style="background: linear-gradient(135deg, #f8b500, #fccb90); padding: 14px 10px; border-radius: 10px; text-align: center; cursor: pointer; border: 1px solid #ffc107; transition: transform 0.2s;">
                <div style="font-size: 22px; margin-bottom: 4px;">🎒</div>
                <div style="color: #121212; font-weight: bold; font-size: 13px;">Todas</div>
                <div style="color: #333; font-size: 11px; margin-top: 2px;">${cacheCatalogoClasses.length} itens</div>
            </div>
        `;
        ...
        listaCategorias.forEach(cat => {
            const qtd = mapaCategorias[cat].length;
            htmlGrid += `
                <div onclick="selecionarCategoriaClasses('${cat.replace(/'/g, "\\'")}')" style="background: #1e1e1e; padding: 14px 10px; border-radius: 10px; text-align: center; cursor: pointer; border: 1px solid #333; transition: transform 0.2s;">
                    <div style="font-size: 22px; margin-bottom: 4px;">📁</div>
                    <div style="color: #fff; font-weight: 600; font-size: 12px; word-break: break-word;">${cat}</div>
                    <div style="color: #8e8e8e; font-size: 11px; margin-top: 2px;">${qtd} item(ns)</div>
                </div>
    ...
    if (termoBusca !== "") {
        if (categoriaAtivaClasses === null) {
            if (nomeCatTxt) nomeCatTxt.textContent = "Pesquisa: Todas as Categorias";
            itensParaExibir = cacheCatalogoClasses.filter(i => 
                (i.nome || "").toLowerCase().includes(termoBusca) || 
                (i.categoria || i.area || "").toLowerCase().includes(termoBusca)
            );
        } else if (categoriaAtivaClasses === "TODAS") {
            if (nomeCatTxt) nomeCatTxt.textContent = "Categoria: Todas";
            itensParaExibir = cacheCatalogoClasses.filter(i => 
                (i.nome || "").toLowerCase().includes(termoBusca)
            );
        } else {
            if (nomeCatTxt) nomeCatTxt.textContent = `Categoria: ${categoriaAtivaClasses}`;
            itensParaExibir = cacheCatalogoClasses.filter(i => 
                (i.categoria || i.area || "Geral") === categoriaAtivaClasses &&
                (i.nome || "").toLowerCase().includes(termoBusca)
            );
        }
    } else {
        if (categoriaAtivaClasses === "TODAS") {
            if (nomeCatTxt) nomeCatTxt.textContent = "Categoria: Todas";
            itensParaExibir = cacheCatalogoClasses;
        } else {
            if (nomeCatTxt) nomeCatTxt.textContent = `Categoria: ${categoriaAtivaClasses}`;
            itensParaExibir = cacheCatalogoClasses.filter(i => (i.categoria || i.area || "Geral") === categoriaAtivaClasses);
        }
    }
    ...
    if (categoriaAtivaClasses === "TODAS" || (categoriaAtivaClasses === null && termoBusca !== "")) {
        const agrp = {};
        itensParaExibir.forEach(item => {
            const c = item.categoria || item.area || "Geral";
            if (!agrp[c]) agrp[c] = [];
        ...
        Object.keys(agrp).sort().forEach(cat => {
            html += `<div style="margin-bottom: 20px;">
                <h4 style="color: #ffc107; margin: 0 0 10px 0; font-size: 14px; border-bottom: 1px solid #262626; padding-bottom: 5px;">📁 ${cat}</h4>
                <div style="display: flex; flex-direction: column; gap: 10px;">
                    ${agrp[cat].map(item => renderizarCardItemCatalogo(item, "classes")).join("")}
                </div>
            </div>`;
        });
        if (containerItens) containerItens.innerHTML = html;
        ...
        if (containerItens) {
            containerItens.innerHTML = `
                <div style="display: flex; flex-direction: column; gap: 10px;">
                    ${itensParaExibir.map(item => renderizarCardItemCatalogo(item, "classes")).join("")}
                </div>
            `;
        }
    }
...
// Renderização Padrão dos Cards nos Catálogos
function renderizarCardItemCatalogo(item, tipo) {
    const imgUrl = item.fotoUrl || "https://res.cloudinary.com/dkozbm1ik/image/upload/v1720640000/avatar-padrao.png";
    return `
        <div style="background: #181818; border: 1px solid #262626; border-radius: 8px; padding: 12px; display: flex; align-items: center; justify-content: space-between; gap: 12px;">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1; min-width: 0;">
                <img src="${imgUrl}" style="width: 42px; height: 42px; border-radius: 6px; object-fit: cover; background: #000; flex-shrink: 0;">
                <div style="min-width: 0; flex: 1;">
                    <div style="color: #fff; font-weight: 600; font-size: 14px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${item.nome || 'Sem nome'}</div>
                    <div style="color: #8e8e8e; font-size: 12px;">${item.categoria || item.area || 'Geral'}</div>
                </div>
            </div>
            <button onclick="iniciarItemPeloCatalogo('${item.id}', '${tipo}')" style="background: #0095f6; color: #fff; border: none; padding: 6px 12px; border-radius: 6px; font-weight: bold; font-size: 12px; cursor: pointer; flex-shrink: 0;">
                Iniciar
            </button>
        </div>
    `;
...
function iniciarItemPeloCatalogo(idItem, tipo) {
    if (typeof iniciarItem === 'function') {
        iniciarItem(idItem, tipo);
    } else {
        alert("Iniciando item: " + idItem);
    }
}
