...
function ajustarLayoutTecladoIOS() {
    const chatContainer = document.getElementById('tela-sala-chat');
    if (!chatContainer || chatContainer.style.display === 'none') return;

    if (window.visualViewport) {
        // Trava a altura na área visível real (acima do teclado)
        chatContainer.style.height = `${window.visualViewport.height}px`;
        chatContainer.style.top = `${window.visualViewport.offsetTop}px`;

        // Rola as mensagens para o final sem mexer no header
        const mensagensContainer = document.getElementById('chat-mensagens-container');
...
if (isIOS && window.visualViewport) {
    window.visualViewport.addEventListener('resize', ajustarLayoutTecladoIOS);
    window.visualViewport.addEventListener('scroll', ajustarLayoutTecladoIOS);
}
...
function abrirSalaChat(usernameAlvo, nomeAlvo, cargoAlvo, fotoAlvo) {
    ...
    // Trava a rolagem de fundo no iOS e mobile
    ...
    if (nomeEl) nomeEl.textContent = nomeAlvo;
    if (cargoEl) cargoEl.textContent = cargoAlvo;
    if (avatarEl) avatarEl.src = fotoAlvo;
